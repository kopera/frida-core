...
#if WINDOWS
		private string? fork_child_pipe_address;
#else
...
#if WINDOWS
				if (fork_child_pipe_address != null) {
					try {
						yield setup_connection_with_transport_uri (fork_child_pipe_address);
					} catch (Error e) {
						assert_not_reached ();
					}
				}
